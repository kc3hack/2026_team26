...
export default function TeamPage(props: TeamProps) {
  ...
  const fetchTeamData = useCallback(async () => {
    ...
    try {
      ...
      // 日付が入力されていれば、Swaggerの要求通り ISO8601 形式(Date-Time)に変換してセット
      if (fromDate) params.f = new Date(`${fromDate}T00:00:00`).toISOString();
      if (toDate) params.t = new Date(`${toDate}T23:59:59`).toISOString();

      const teamDataRes = await apiClient.get('/team/fatigue', { params }); // paramsを渡す
  ...
  const handleCreateTeam = async () => {
    ...
    setIsActionLoading(true); // ← UIのためにローディングを追加
    setErrorMsg(null);

    try {
      const body: TeamCreateReq = { name: createName };

      await apiClient.post<Team>('/team/create', body);
  ...
  const handleJoinTeam = async () => {
    ...
    try {
      ...
      await apiClient.post<Team>('/team/join', body);
